...
// Helper function to calculate angle between three points (from your original `calcAngle` with minor naming change for consistency)
...
// JumpingJackDetection component
...
export default function JumpingJackDetection({
  isActive,
  onDetectionUpdate = () => {},
}) {
  ...
  const jackCountRef = useRef(0); // Renamed from jackCount for consistency
  const stageRef = useRef(null); // Tracks the current stage of the exercise ("in", "out", or null)
  ...
  const renderLoop = useCallback(() => {
    ...
    if (video.currentTime !== lastVideoTimeRef.current) {
      ...
      if (results.landmarks && results.landmarks.length > 0) {
        ...
        // Key points for Jumping Jack detection
        ...
        if (
          ...
          // Debug values (these are for drawing on canvas, also useful for passing to parent)
          const wristAboveHead =
            leftWrist[1] < nose[1] && rightWrist[1] < nose[1];
          const shoulderDistance = Math.abs(leftShoulder[0] - rightShoulder[0]);
          const ankleDistance = Math.abs(leftAnkle[0] - rightAnkle[0]);

          // Counting logic
          if (wristAboveHead && ankleDistance > 1.5 * shoulderDistance) {
            if (stageRef.current !== "out") {
              stageRef.current = "out"; // Arms/legs are 'out'
            }
          } else if (
            !wristAboveHead &&
            ankleDistance < 1.2 * shoulderDistance &&
            stageRef.current === "out"
          ) {
            jackCountRef.current += 1; // Increment count
            console.log("Jumping Jacks:", jackCountRef.current);
            stageRef.current = "in"; // Arms/legs are 'in'
          }
          ...
          onDetectionUpdate({
            reps: jackCountRef.current,
            stage: stageRef.current,
            status: "Detecting jumping jacks...",
            // You might want to pass these for display/debugging in AICounterPage if needed
            // wristAboveHead,
            // shoulderDistance,
            // ankleDistance
          });

          // Draw count and debug values directly on the canvas (text is NOT mirrored)
          ctx.font = "bold 30px Arial";
          ctx.fillStyle = "lime";
          ctx.fillText(`Jumping Jacks: ${jackCountRef.current}`, 20, 50); // Position at top-left
          ctx.font = "20px Arial";
          ctx.fillText(`Ankle Dist: ${ankleDistance.toFixed(0)}`, 20, 80);
          ctx.fillText(
            `Shoulder Dist: ${shoulderDistance.toFixed(0)}`,
            20,
            105
          );
          ctx.fillText(`Stage: ${stageRef.current || "null"}`, 20, 130);
        } else {
  ...
  useEffect(() => {
    const initializeLandmarker = async () => {
      try {
        console.log(
          "JumpingJackDetection: Initializing PoseLandmarker model..."
        );
        ...
        console.log("JumpingJackDetection: PoseLandmarker model initialized.");
      } catch (error) {
        console.error(
          "JumpingJackDetection: Error initializing PoseLandmarker model:",
          error
        );
        onDetectionUpdate({
          reps: 0,
    ...
    return () => {
      console.log(
        "JumpingJackDetection: Model initialization cleanup running."
      );
      if (poseLandmarker) {
  ...
  useEffect(() => {
    ...
    const stopCameraAndLoop = () => {
      console.log(
        "JumpingJackDetection: Stopping camera and detection loop..."
      );
      ...
      jackCountRef.current = 0; // Reset count when stopping
      stageRef.current = null; // Reset stage when stopping
    ...
    if (isActive && currentLandmarker) {
      ...
      if (!isCameraPlayingRef.current) {
        console.log(
          "JumpingJackDetection: isActive is true and PoseLandmarker is ready. Attempting to start camera..."
        );
        onDetectionUpdate({
          reps: 0,
          stage: null,
        ...
        const startCamera = async () => {
          try {
            console.log(
              "JumpingJackDetection: Calling navigator.mediaDevices.getUserMedia..."
            );
            // Request camera access with ideal high resolution for better detection quality
            const stream = await navigator.mediaDevices.getUserMedia({
              video: { width: { ideal: 1920 }, height: { ideal: 1080 } },
            });
            console.log("JumpingJackDetection: Camera stream obtained.");
            ...
            if (currentVideo) {
              ...
              if (!currentVideo.onloadedmetadata) {
                currentVideo.onloadedmetadata = () => {
                  console.log(
                    "JumpingJackDetection: Video metadata loaded. Playing video."
                  );
                  currentVideo.play(); // Start video playback
                  isCameraPlayingRef.current = true; // Set camera status to playing
                  console.log(
                    "JumpingJackDetection: Camera stream acquired and playing."
                  );
                  onDetectionUpdate({
                    reps: 0,
                    stage: null,
                ...
                // If onloadedmetadata was already set (e.g., video re-used), just play and start loop
                currentVideo.play();
              ...
              console.warn(
                "JumpingJackDetection: videoRef.current was null when trying to set srcObject. This might indicate a rendering issue."
              );
              onDetectionUpdate({
                reps: 0,
                stage: null,
            ...
            console.error(
              "JumpingJackDetection: Error starting camera:",
              error
            );
        ...
        console.log(
          "JumpingJackDetection: Camera already active, awaiting detection loop."
        );
        onDetectionUpdate({
          reps: jackCountRef.current,
          stage: stageRef.current,
    ...
    return () => {
      console.log(
        "JumpingJackDetection: Main camera/detection effect cleanup running."
      );
  ...
  return (
    ...
    >
      ...
      {isActive && !poseLandmarker && (
        <p style={{ color: "var(--text-color)" }}>
          Loading Jumping Jack Detection Model...
        </p>
      )}
      {isActive &&
        poseLandmarker &&
